...
const SearchUser = ({ UID }) => (
  <div className={classes.searchPageBody}>
    ...
    {UID ? (
      withFirebase(withRouter(SearchUserBaseClass))({ UID })
    ) : (
      <NotLoggedIn />
    )}
  </div>
...
class SearchUserBaseClass extends Component {
  ...
  onSearch = event => {
    ...
    this.props.firebase
      ...
      .startAt(this.state.searchuser)
      .endAt(this.state.searchuser + "\uf8ff")
      .on("value", snapshot => {
...
const Searcher = ({ searchResult, friendlist, onAddFriend, myID }) => (
  <React.Fragment>
    {Object.keys(searchResult).map(user => (
      <ResultUser
        key={user}
        myID={myID}
        userID={user}
